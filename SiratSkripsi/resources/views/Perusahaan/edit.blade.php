...
<div class="container mt-4">
    ...
    <form action="{{ route('perusahaan.update', $data_perusahaan->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="nama_cabang" class="form-control" value="{{ old('nama_cabang', $data_perusahaan->nama_cabang) }}" required>
        </div>
        <div class="mb-3">
            <label for="kota_kabupaten" class="form-label">Kota/Kabupaten</label>
            <input type="text" name="kota_kabupaten" class="form-control" value="{{ old('kota_kabupaten', $data_perusahaan->kota_kabupaten) }}" required>
        </div>
        <div class="mb-3">
            <label for="alamat" class="form-label">Alamat</label>
            <input type="text" name="alamat" class="form-control" value="{{ old('alamat', $data_perusahaan->alamat) }}" required>
        </div>
        <div class="mb-3">
            <label for="nama_pimpinan" class="form-label">Nama Pimpinan</label>
            <input type="text" name="nama_pimpinan" class="form-control" value="{{ old('nama_pimpinan', $data_perusahaan->nama_pimpinan) }}" required>
        </div>
        <div class="mb-3">
            <label for="nib_cabang" class="form-label">NIB Cabang</label>
            <input type="text" name="nib_cabang" class="form-control" value="{{ old('nib_cabang', $data_perusahaan->nib_cabang) }}" required>
        </div>
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            @if ($data_perusahaan->pdf_akta_cabang)
                <p class="mt-2">
                    <a href="{{ asset('storage/' . $data_perusahaan->pdf_akta_cabang) }}" target="_blank">Lihat PDF Akta Cabang saat ini</a>
                </p>
            @endif
        </div>
