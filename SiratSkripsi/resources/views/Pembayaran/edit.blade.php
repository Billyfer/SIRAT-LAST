...
<div class="container mt-4">
    ...
    <form action="{{ route('pembayaran.update', $pembayaran->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <select name="id_data_jamaahs" id="id_data_jamaahs" class="form-control" required>
                @foreach($jamaahs as $jamaah)
                    <option value="{{ $jamaah->id }}" {{ $pembayaran->id_data_jamaahs == $jamaah->id ? 'selected' : '' }}>
                        {{ $jamaah->nama_jamaah }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <input type="text" name="penerima" id="penerima" class="form-control" value="{{ $pembayaran->penerima }}" required>
        </div>
        <div class="mb-3">
            <label for="bukti_pembayaran" class="form-label">Bukti Pembayaran</label>
            <input type="file" name="bukti_pembayaran" id="bukti_pembayaran" class="form-control">
            @if($pembayaran->bukti_pembayaran)
                <small><a href="{{ asset('storage/' . $pembayaran->bukti_pembayaran) }}" target="_blank">Lihat Bukti Pembayaran</a></small>
            @endif
        </div>
